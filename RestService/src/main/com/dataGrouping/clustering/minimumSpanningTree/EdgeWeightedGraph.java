 ...
 *  @author Kevin Wayne
 *  adapted by Erwin to use this class with the distance matrix.
 */
...
//    }
    ...
    /**
     * Creates an weighted graph from an input triangular matrix. Avoids self-loops.
     * @param matrix triangular matrix
     */
    public EdgeWeightedGraph(double[][] matrix){
    this(matrix.length);
    int V = matrix.length;
    
    // run through the triangular matrix
    double weight = 0;
    for(int i = 0; i < V; i++){
    	for(int j = i; j >=0; --j){
    			// TODO this class should be more self-consistent.
    		weight = matrix[i][j];
